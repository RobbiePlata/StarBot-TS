...
def process(replay):
    ...
    try:
        if winner in names:
            ...
            if winnerrace == "Zerg":
                print("Loss vs Protoss detected")
                readVsProtossfile = open("Statistics/VsProtoss.txt", "r")
                VsProtoss = readVsProtossfile.read()
                wins = int(VsProtoss.split('-')[0])
                losses = int(VsProtoss.split('-')[1])
